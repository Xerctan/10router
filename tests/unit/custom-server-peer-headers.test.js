// custom-server.js is the only thing that makes x-10r-real-ip trustworthy. Boot a real
...
describe("custom-server peer header sanitizing", () => {
  it("generates a peer trust token at boot", () => {
    expect(process.env.TENROUTER_PEER_TOKEN).toMatch(/^[0-9a-f]{48}$/);
  });

  it("replaces a client-supplied x-10r-real-ip with the socket address", async () => {
    const headers = await get({ "x-10r-real-ip": "203.0.113.55" });

    expect(headers["x-10r-real-ip"]).toMatch(/^(::ffff:)?127\.0\.0\.1$/);
  });

  it("stamps the trust token so downstream can tell the wrapper ran", async () => {
    const headers = await get();

    expect(headers["x-10r-peer-token"]).toBe(process.env.TENROUTER_PEER_TOKEN);
  });

  it("drops a client-supplied peer trust token", async () => {
    const headers = await get({ "x-10r-peer-token": "forged-token" });

    expect(headers["x-10r-peer-token"]).toBe(process.env.TENROUTER_PEER_TOKEN);
    expect(headers["x-10r-peer-token"]).not.toBe("forged-token");
  });

  it("drops a client-supplied x-10r-via-proxy marker", async () => {
    const headers = await get({ "x-10r-via-proxy": "1" });

    expect(headers["x-10r-via-proxy"]).toBeUndefined();
  });
  ...
  it("marks via-proxy and adopts the forwarded IP for a loopback proxy hop", async () => {
    ...
    expect(headers["x-10r-via-proxy"]).toBe("1");
    expect(headers["x-10r-real-ip"]).toBe("203.0.113.9");
  ...
  it("keeps the peer token out of persisted request details", () => {
    const sanitized = requestDetails.sanitizeHeaders({
      "x-10r-peer-token": "secret",
      "x-10r-cli-token": "secret",
      "x-9r-cli-token": "secret",
      "authorization": "Bearer sk-x",
      "x-10r-real-ip": "127.0.0.1",
    });

    expect(sanitized["x-10r-peer-token"]).toBeUndefined();
    expect(sanitized["x-10r-cli-token"]).toBeUndefined();
    expect(sanitized["x-9r-cli-token"]).toBeUndefined();
    expect(sanitized["authorization"]).toBeUndefined();
    expect(sanitized["x-10r-real-ip"]).toBe("127.0.0.1");
  });
});
