// x-10r-real-ip is only trustworthy when custom-server.js stamped it from the TCP socket.
...
export function hasTrustedPeerHeaders(request) {
  const token = process.env.TENROUTER_PEER_TOKEN;
  return Boolean(token) && request.headers.get("x-10r-peer-token") === token;
}
