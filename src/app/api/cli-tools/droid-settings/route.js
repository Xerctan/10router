...
// Check if settings has 10Router customModels
const has10RouterConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:10Router"));
};
...
// POST - Update 10Router customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing 10Router configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:10Router"));
...
// DELETE - Remove 10Router customModels only (keep other settings)
