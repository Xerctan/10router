...
// POST - Apply 10Router config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing 10Router entry or append
    const idx = config.findIndex((e) => e.name === "10Router");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove 10Router entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "10Router");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "10Router removed from Copilot config",
    });
  } catch (error) {
