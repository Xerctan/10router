...
>
> **⚠️ 本文是 2026-09-11 的历史快照，主体内容已过时。** 下方各节保留的是当时的
> 分析与结论，**不要再按它排期**。截至 v1.2.0（2026-09-23）已复核的实际状态：
>
> | 项 | 2026-09-11 记为 | 现况 |
> |---|---|---|
> | #9-2 凭据加密落库 | ❌ 未修 | ✅ 已修（migration 003 + `enc:v1:` AES-GCM） |
> | #9-3 默认口令兜底 | ✅ 已修(1) | ✅ 已修，且 `login` 路由内已无 `123456` / `DEFAULT_PASSWORD` |
> | #9-4 requireLogin 横幅与二次确认 | ❌ 未修 | ✅ 已修（SecurityCard / 常驻横幅） |
> | #9-8 会话时长 | ❌ 未修 | ✅ 已改（24h→2h + 滑动续期）；**限流持久化仍开**（内存 `Map`，重启清零） |
> | #9-11 `/api/version` 移入鉴权 | ❌ 未修 | 🔵 **刻意保留公开**（健康检查 / 更新轮询需要） |
> | #9-6 更新器完整性校验 | ❌ 未修 | 🟡 CORS 已收紧；**`npm i -g …@latest` 仍无哈希校验** |
> | #10 内容过滤重试 | ⏸ 未实现 | ✅ 已实现（`combo.js` 接 `sensitiveRetry.gateResponse`） |
> | #13 桌面版模型报错分类 | ⏸ 下版待办 | ✅ 已修（`mimo-desktop-session-error.test.js` 锁死） |
> | #14-A 补刷禁用列表 | ⏸ 待办 | ✅ 已修（`refreshAfterConnectionChange`） |
> | #14-B 禁用区复用 ModelRow | ⏸ 待办 | ✅ 已修（带 `onTest`） |
> | `gpt-6-astra` 倍率 | 未进产物 | ✅ 已在码（`6.67`） |
>
> **仍未关闭的只剩两条**：`#9-6`（更新器无哈希/完整性校验）与 `#9-8`（登录限流未持久化）。
> 两者都记在 `docs/zh-CN/release-review-v1.2.0.md`。
> #14-C（禁用区「测试全部」批量）仍为可选加分项。
...
| 更新日志 | v1.1.0 那条补注「⚠️ 该值后经实测修正为 `6.67`」，标题改为「含一个**后经修正的**估算倍率」；顶部新增 `## v1.1.1 — 未发布` 完整记录 |
