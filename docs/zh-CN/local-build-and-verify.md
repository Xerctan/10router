...
> 早期只按 tag 推导，得到的是常量（tag 不动就永远是 `1.1.4-test.1`）—— 一轮轮热替换的不同产物
> 于是共用一个版本号，正是 §1 那条「一个号三份产物」的坑。**别用时间戳/随机数去“修”它**：
> 号虽然也唯一，但一串 unix 秒根本看不出是第几轮、每轮都不同这件事也不直观。
> 想让界面直接显示工作树版本就 `-Version 1.1.4`（但别把它当默认：同一版本号下的产物会再次变得分不清）。
...
事故复盘**一篇一文件**，总索引（按发生面分类：本地测试轮 / NAS 热替换 / 发版与 CI，
并收编既有专项复盘文档）：**[`test-report-INDEX.md`](test-report-INDEX.md)**。

- 本手册只留浓缩后的闸门（如 §2.4 的 SSR 冒烟）与速查表（§6）；历史叙事在报告文件里；
- 写新报告的时机与收尾三步见索引文件开头。
