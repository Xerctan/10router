...
- **mirasim 输入订正脚本不再重复加缓存**：`normalize-mirasim-input.mjs` 只认 `mirasimInputNormalized` 标记跳过，但修正后的转换器（插件 export-usage ≥ v1.5.0、CreditDaddy 用量同步）写入时已是 `prompt = input + cache` 却**不打标记**——同步后再跑脚本会把缓存再加一遍（638 + 1.13 亿 → 638 + 2.26 亿），行与日桶一起虚高。现在按数据本身识别：`promptTokens ≥ cacheRead + cacheWrite` 即已是新口径，跳过（代价：净新增输入本就大于缓存的旧行也被跳过，少计至多其自身输入，远小于整份缓存重复）；插件转换器写入时也补上标记。dedup 签名不含 meta，加标记不会触发重导。用例 `normalize-mirasim-input.test.js`（子进程跑真实脚本：旧行加一次、CreditDaddy 式无标记新行不动、日桶只加一份、幂等）。
