...
- **StepFun Native Integration**:
  - **China / International / Step Plan channels split**: `stepfun-cn` (China pay-as-you-go, cash/voucher), `stepfun` (international pay-as-you-go, `api.stepfun.ai`), and `stepfun-plan-cn` / `stepfun-plan` (Step Plan subscription — **spends plan Credits, never your balance**, includes the plan-only smart router `step-router-v1`; usable directly from Claude Code). Existing connections are migrated to `stepfun-cn` automatically.
