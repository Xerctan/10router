...
- **[重要] 開啟登入檢查後被鎖在外面（#33）**：尚未設定自己的密碼時開啟「需要登入」，之後只有那個看不到的首次登入密碼有效（fnOS 安裝時自動產生、寫在你看不到的檔案中），輸入什麼都提示密碼錯誤。現在未設定密碼時不允許開啟登入檢查、拒絕空密碼，並新增找回入口：登入頁 **「忘記密碼？」**。fnOS 直接在 **「應用中心 → 10Router → 應用設定」** 填寫新密碼並儲存即可；其他安裝方式在資料目錄放一個 `reset-password` 檔案。

### ✨ 新增

- **CreditDaddy 接入：額度總覽唯讀介面**：`GET /api/usage/quotas` 一次回傳全部可查額度的連線，口徑與儀表板「額度」頁一致，可用儀表板虛擬 key（`sk-…`）存取。結果快取 5 分鐘；強制重新整理（`?force=1`）對同一連線每 30 秒最多一次，同時到達的請求共用一次上游查詢。不回傳任何憑證——但帳號電子郵件對持有虛擬 key 的人可見，分發 key 前請知悉。
- **匯入的用量開始顯示預估成本**：經 10router-sync 外掛或 CreditDaddy 同步進來的用量先前全是 $0。新匯入依即時價目表估算，歷史資料於啟動時自動補算（絕不覆寫來源已算好的成本；沒有價格的模型仍為 $0）。
- **「登入檢查已關閉」橫幅可以隱藏**：設定 → 安全中「需要登入」下方新增開關，僅在登入檢查關閉時出現。隱藏需要確認；重新開啟登入檢查後再關閉，橫幅會自動恢復。
- **可以關閉自動檢查更新**（設定 → 安全）：適合刻意停留在某個版本的安裝。關閉後 10Router 不再自行連線更新伺服器、不再提示新版本——儀表板、系統匣氣泡與 CLI 啟動器都會靜默；「立即檢查」與系統匣「檢查更新」仍可手動使用。
- **單一認證卡片支援 OAuth 加密匯入 / 匯出**：遷移按鈕現在也出現在僅 OAuth 的卡片上——尤其是 **MiMo Desktop**，它的登入工作階段本身就是換機時要遷移的憑證。
- **Qwen 官方精確價**：新一代 Qwen 旗艦先前按低價萬用字元檔計費（如 qwen3.8-max 只算了官方價的四分之一），現依官方逐模型價格計算。

### 🐛 修正

- **[重要] 重新啟動後遠端存取一直中斷，直到有人開啟儀表板**：通道 / Tailscale / MITM 自動恢復等啟動任務先前要等第一次渲染頁面才執行——只透過 `/v1` 使用的 NAS 可能一直等不到。現在隨服務一起啟動。
- **Qoder 排隊限流被當成模型回覆**：排隊中的請求（`10605`）以 `[qoder error 403: …]` 文字回傳給用戶端，還被記為成功。現在會切換到下一個帳號，並依 Qoder 要求的時長（約 30 秒）冷卻被限流的帳號。
- **免密碼儀表板在本機匯入 OAuth 遷移檔被攔截**：現已恢復。
- **10router-sync 外掛 v1.5.0**：適配 ZCode 新的套餐渠道 id（Start Plan 流量自 09-18 起漏同步）；mirasim 輸入計入快取，儀表板不再出現「輸入 638、快取 1.13 億」；mirasim 訂正腳本不再重複加上快取。

### 🔧 其他

- **認證請求標頭對齊 10Router 命名**：`x-10r-cli-token` / `x-10r-password` 取代沿用下來的 `x-9r-*`。舊名稱仍然相容，既有的 CLI 啟動器、外掛與腳本不受影響。

## v1.2.0 (2026-09-24)

### ✨ 新增

- **小米 MiMo 拆成三張卡，各管各的帳**：雲端（`xiaomi-mimo`，瀏覽器登入 / API Key，按正式資費）、桌面版（`mimo-desktop`，必須由本機已登入的 MiMo Desktop 用戶端提供工作階段，不接受 API Key）、Token Plan（`tp-` 套餐金鑰）。先前一張卡同時管兩類憑證，「週額度」與「測試連線」都按最寬鬆的一側判定——雲端卡會顯示它其實讀不到的桌面版週額度。現在雲端卡不再顯示週額度，桌面版卡獨占該權益；v1.1.3 時期建錯的既有連線由遷移自動歸位。
- **MiMo V2.6 上線、V2.5 退役**：接入 V2.6 全系列，並為 MiMo 模型宣告**真實上下文視窗**（1M / 128K 輸出；先前落於預設值以猜測計算）。**V2.6 Pro UltraSpeed** 亦加入雲端與 Token Plan 卡——訂製服務，有合約即可直接使用，無合約會收到上游錯誤，無需手動新增模型。
- **StepFun（階躍星辰）全系列接入**：國內站 / 國際站 × 按量 / Step Plan 四渠道，短別名 `step-cn` / `step` / `stepp-cn` / `stepp`；Step Plan 渠道消耗套餐 Credit、提供 Anthropic 原生介面（Claude Code 可直連），並有套餐專屬 `step-router-v1` 智慧路由；LLM / 語音合成 / 語音識別選單嚴格隔離（圖像服務官方已公告停服，故不放出）；即時餘額與代金券查詢。
- **ComfyUI 本機生圖**：連接本機 ComfyUI 後自動探索 Checkpoints、組裝 SD / SDXL / Flux 文生圖工作流，經 `/v1/images/generations` 回傳標準圖像。
- **媒體供應商清單支援拖曳排序、已連線自動前置**：與主供應商頁共用同一套排序與持久化，`/v1/models` 順序保持一致。
- **組合（combo）新增「空回覆時切換」策略**（預設關）：部分上游將內容審查表達為 HTTP 200 空流，現在會自動落到下一個模型；單請求預設最多連切 2 個——重放會重新計費完整輸入上下文，高上下文組合慎用。
- **逐個測試連線可選模型**：測試下拉可挑選具體模型（預設「供應商預設」），因為某些憑證只涵蓋部分模型——桌面版工作階段就只到桌面模型。Qoder 的逐個測試改為依**該帳號的即時模型目錄**核對別名（不消耗點數），且 Qoder 國內版不再回傳「不支援」。
- **「隱藏無配額」開關移入「設定 → 實驗性」**：配額頁工具列先前按鈕過於擁擠，該開關現收納於實驗性頁的供應商卡片中。

### 🔒 安全

- **不再有內建預設密碼**：未設定密碼（且未設定 SSO）時儀表板**只能在本機開啟**，遠端請求會被明確拒絕並提示設定路徑；`INITIAL_PASSWORD` 是唯一的非互動式引導口令。**升級前若一直用預設口令從其它裝置登入，請先在本機設定一次密碼。**
- **上游憑證加密儲存**：OAuth token / API key 以 AES-256-GCM 加密落庫，金鑰存放在資料庫之外（`$DATA_DIR/credential-key`，或以 `CREDENTIAL_SECRET` 指定）。資料庫檔案被複製 / 同步不再等於憑證外洩。**金鑰檔請與資料庫一起備份——遺失金鑰後憑證無法還原。**
- **用量日誌不再記錄完整 API key**：只保留可讀前綴與不可逆摘要，按 key 統計照舊運作。
- **新增「安全」卡片（設定 → 實驗性）**：監聽位址、密碼狀態、登入檢查、存取範圍與憑證儲存狀態集中展示，並提供「**儀表板僅本機存取**」開關（請求級即時生效，不影響 `/v1` 閘道）。未設密碼或關閉登入檢查時全頁面常駐警告橫幅；關閉「要求登入」需二次確認。
- **登入工作階段收緊**：時長從 24 小時縮短為 2 小時並滑動續期（使用中不會被打斷）；本版再加**絕對上限**——自首次登入起超過 30 天必須重新登入，被盜工作階段無法靠持續存取無限續期。
- **fnOS 升級不再重置引導密碼**：隨機口令改存到跨升級保留的資料目錄，先前每次 fpk 升級都會換掉密碼、把依賴舊口令的使用者鎖在外面。
- **備份匯出不再靜默遺失憑證**：在金鑰缺失 / 錯誤的機器上匯出的備份先前不含任何憑證卻提示「下載成功」；現在原樣保留密文並明確告警「需以原始金鑰才能還原」。
- **`/api/health` 不再回傳本機路徑**：該公開介面先前會把驅動載入失敗的原始錯誤（含本機絕對路徑）一併回傳。
- **Windows 金鑰檔補齊權限收緊**：加密金鑰與登入金鑰改用 ACL 僅本人可讀（與先前 Root CA 私密金鑰一致）；MITM 的 sudo 密碼也不再使用寫死在原始碼裡的備援金鑰。

### 🐛 修正

- **[重要] 用錯金鑰啟動會永久銷毀既有憑證**：把資料庫還原到未攜帶金鑰的機器上啟動一次，所有 OAuth token 與 API Key 的密文會被抹除，之後換回正確金鑰也救不回。現改為原樣保留密文，換回金鑰即可還原。
- **[重要] 部分回應串流被拼接錯誤**：stop 序列守衛注入的補發列缺少分隔，會與終止幀併成同一個事件，使部分用戶端（含官方 SDK）解析失敗；Anthropic 回應另缺事件類型列、內容區塊序號寫死。現全部修正，並在命中後整塊丟棄非文字區塊（先前會殘留一個參數為空的工具呼叫，用戶端可能空參執行）。
- **[重要] 純工具呼叫的回覆被誤判為「空回覆」**：開啟「空回覆時切換模型」後，模型一旦決定呼叫工具就被當成沒有輸出，退回下一個模型並**重新計費整個輸入上下文**。現已正確識別工具呼叫與推理內容，且被放棄的那次呼叫用量照常入帳。
- **Claude Code 自動模式分類器經閘道恆不可用（#18）**：部分上游接受 `stop` / `stop_sequences` 卻無視它。閘道現在於串流轉發層代用戶端執行 stop 契約（序列本身不發出、保留用量統計、把被截斷的 `length` 改寫為正常 stop），並補齊雙向 stop 參數對應。
- **小米 MiMo 瀏覽器登入在區域網路 / HTTP 下完全不可用**：OAuth `state` 產生先前依賴只存在於安全上下文的 API，從其它裝置以 `http://區域網路IP` 開啟時（NAS 的常態用法）點擊「瀏覽器」直接報錯，整條登入鏈路被堵死。
- **SiliconFlow 中國區的別名被劫持**：`sfcn` 同時被兩家供應商宣告，導致 SiliconFlow 的請求被打到 StepFun。現已歸還原主，並新增全域別名唯一性檢查防止再犯。
- **StepFun 四渠道測試連線回報「不支援」**：健康金鑰也一律失敗；現走通用 OpenAI 相容驗證。
- **Token Plan 瀏覽器登入後打錯叢集**：套餐帳號的 chat / TTS / 測試連線先前一律打到按量叢集；現依平台回傳並儲存的端點路由。
- **配額頁顯示與操作問題**：有檢視篩選時「顯示中 1-10 of 46」與實際卡片數不符（現按實際顯示數量回報）；同一張卡內連續快速點擊多個「已隱藏」標籤會遺失大部分操作（點 5 個只還原 2 個），現已全部生效；「只看有餘額」與手動隱藏重新打通——批次隱藏的列現在也出現在「已隱藏」清單中、可按名稱逐一點回。
- **用量按金鑰統計把同一台機器的所有金鑰合併成一桶**：分組鍵先前使用遮蔽字串，而同一台機器所有金鑰的遮蔽值相同；現改用金鑰摘要分組。
- **MiMo 其他修正**：週額度用盡時不再把上游原始 JSON 鋪進連線列；TTS 裸模型名不再被靜默改寫為預設模型；瀏覽器登入掛起工作階段的金鑰數與桌面版對齊上限 8。

### 🔧 其他

- 還原 258 條因品牌更名而失效的介面翻譯（先前靜默顯示英文）。
- 到期 / 重置倒數不再跨天被折疊：41 小時不再顯示成 `1d`，統一為 `1d 17h` / 「1天17小時」。
- 儀表板 Change Log 只显示已發布的版本段，不會提前看到開發中版本的功能。
- 安裝包與文件不再把使用者導向無關的 fork。
