...
- **不再有內建預設密碼**：過去未設定密碼的實例，任何人都能用預設口令從區域網路一步登入管理介面。現在**沒有預設口令** —— 未設定密碼（且未設定 SSO）時儀表板**只能在本機開啟**，遠端請求會被明確拒絕並提示你到本機設定密碼；無頭部署（Docker / fnOS）以 `INITIAL_PASSWORD` 指定初始口令。**升級後若先前一直用預設口令從其它裝置登入，請先在本機設定一次密碼。**
- **上游憑證加密儲存**：供應商的 OAuth token / API key 不再以明文寫在資料庫裡（AES-256-GCM，金鑰存放在資料庫之外）。資料庫檔案被備份、同步或複製走不再等於憑證外洩。金鑰檔請與資料庫一起備份，或設定 `CREDENTIAL_SECRET`。
- **用量日誌不再記錄完整 API key**：用量表與每日聚合只保留可讀前綴（`sk-496f00***`）與不可逆摘要，按 key 統計與命名照舊運作。
- **新增「安全」卡片**（設定 → 實驗性）：一眼看清監聽位址與區域網路可達位址、密碼是否已設定、登入檢查是否開啟、存取範圍、憑證儲存狀態；並提供「**儀表板僅本機存取**」開關（請求級生效）。未設密碼或關閉登入檢查時，儀表板頂端會常駐警告橫幅。
- **其餘稽核項**：關閉「要求登入」需二次確認；登入工作階段從 24 小時縮短為 2 小時並**滑動續期**（使用中不會被打斷）；`/api/version`、`/api/init` 不再對網路公開（本機 CLI / 托盤照舊）；MITM Root CA 私鑰在 Windows 上也收緊為僅本人可讀，MITM 的 sudo 密碼加密不再使用寫死在原始碼裡的備援金鑰。

### ✨ 新增

- **MiMo Token Plan 頻道**：卡片更名為「MiMo Token Plan」，搭配官方圖示；額度列不再顯示「未實作」—— 該頻道官方沒有公開額度介面，現在會明確說明，並在連線同時帶有桌面端帳號工作階段時照舊顯示週額度。
- 供應商橫幅與新增安全文案全部補齊簡繁中文。

### 🛠️ 優化與修正

- 修正安全卡片在**完全加密**的資料庫上誤報「加密失敗」的問題（空代理位址被當成明文金鑰）。
- 修正備份檔案鏈路：下載的備份先前會寫成無法還原的密文，還原舊備份又會把憑證變回明文 —— 現在匯出可攜、匯入即加密。
- 備份介面新增提示：備份檔包含可讀憑證，請妥善保管。
