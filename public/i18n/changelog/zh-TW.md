...
- **小米 MiMo 拆成三張卡，各管各的帳**：雲端（`xiaomi-mimo`，瀏覽器登入 / API Key，按正式資費）、桌面版（`mimo-desktop`，必須由本機已登入的 MiMo Desktop 用戶端提供工作階段，不接受 API Key）、Token Plan（`tp-` 套餐金鑰）。先前一張卡同時管兩類憑證，導致「週額度」與「測試連線」都按最寬鬆的一側判定——雲端卡會顯示它其實讀不到的桌面版週額度。現在雲端卡**不再顯示週額度**，桌面版卡獨占該權益；「測試連線」改用即時模型探測，不再因一個已下架的模型 id 對新卡永遠失敗。v1.1.3 時期建錯的既有連線會由遷移自動歸位。
- **MiMo V2.6 上線、V2.5 退役**：接入 V2.6 全系列，並為 MiMo 模型宣告**真實上下文視窗**（1M / 128K 輸出；先前落於預設值，以猜測計算）。**V2.6 Pro UltraSpeed** 亦加入雲端與 Token Plan 卡——它是訂製服務，有合約即可直接使用，無合約會收到上游錯誤，無需手動新增模型。
- **逐個測試連線可選模型**：測試下拉可挑選具體模型（預設「供應商預設」），因為某些憑證只涵蓋部分模型——桌面版工作階段就只到桌面模型。Qoder 的逐個測試改為依**該帳號的即時模型目錄**核對別名（不消耗點數），且 Qoder 國內版不再回傳「不支援」。
- **「隱藏無配額」開關移入「設定 → 實驗性」**：配額頁工具列先前按鈕過於擁擠，該開關現收納於實驗性頁的供應商卡片中（用戶端檢視偏好，位置更合理）。

### 🐛 修正

- **[重要] 用錯金鑰啟動會永久銷毀既有的憑證**：把資料庫還原到未攜帶金鑰的機器上啟動一次，所有 OAuth token 與 API Key 的密文會被抹除，之後換回正確金鑰也救不回。現已改為原樣保留密文，換回金鑰即可還原。
- **[重要] 部分回應串流被拼接錯誤**：stop 序列守衛注入的補發列缺少分隔，會與終止幀併成同一個事件，使部分用戶端（含官方 SDK）解析失敗。觸發條件是文字以某個停止詞的前綴結尾，與上游是否規範無關；Anthropic 協定的回應另修正了事件類型列與內容區塊序號，並在命中後整塊丟棄非文字區塊（先前會殘留一個參數為空的工具呼叫，用戶端可能空參執行）。
- **[重要] 純工具呼叫的回覆被誤判為「空回覆」**：開啟「空回覆時切換模型」後，模型一旦決定呼叫工具就會被當成沒有輸出，退回下一個模型並**重複計費整個輸入上下文**。現已正確識別工具呼叫與推理內容，且被放棄的那次呼叫用量照常入帳。
- **SiliconFlow 中國區的別名被劫持**：`sfcn` 這個短別名同時被兩家供應商宣告，導致 SiliconFlow 的請求被打到 StepFun。現已歸還原主，並新增全域別名唯一性檢查防止再犯。
- **配額頁兩個顯示問題**：有檢視篩選時，「顯示中 1-10 of 46」與實際卡片數不符（現按實際顯示數量回報）；同一張卡內連續快速點擊多個「已隱藏」標籤會遺失大部分操作（點 5 個只還原 2 個），現已全部生效。
- **「只看有餘額」與手動隱藏重新打通**：該開關先前自成一派——它隱藏的列不會出現在「已隱藏」清單中，也無法單獨還原。現兩者共用同一份狀態，批次隱藏的列照樣列出、照樣能按名稱點回來。
- **用量按金鑰統計把同一台機器的所有金鑰合併成一桶**：分組鍵用的是遮蔽後的雜湊片段，而該片段在同一台機器上對所有金鑰都相同。現改用金鑰摘要分組。
- **fnOS 每次升級都會換掉引導密碼**：隨機口令先前寫在升級時會被整體替換的目錄中，升級後重新產生，依賴舊口令的使用者被靜默鎖死。現改存到跨升級保留的資料目錄，升級不再更動它。
- **工作階段無法真正過期**：滑動續期先前只看目前權杖，被盜用的登入狀態只要持續存取就能無限延長。現記錄首次登入時間（已簽章，用戶端無法竄改），超過 30 天必須重新登入。
- **備份匯出在金鑰錯誤時靜默遺失憑證**：匯出的備份不含任何憑證卻提示「下載成功」。現會明確告警，說明這些憑證需以原始金鑰才能還原。
- **/api/health 不再回傳本機路徑**：該介面公開可存取，先前會把驅動載入失敗的原始錯誤（含本機絕對路徑）一併回傳。
- **Windows 上的金鑰檔補齊權限收緊**：加密金鑰與登入金鑰先前只設定了類 Unix 權限位（在 Windows 上無效），現與 Root CA 一致改用 ACL。

### 🔧 其它

- 還原 258 條因品牌更名而失效的介面翻譯（先前靜默顯示英文）。
- 到期倒數不再跨天被折疊：41 小時不再顯示成 `1d`，統一為 `1d 17h` / 「1天17小時」。
- MiMo 週額度用盡時不再把上游的原始 JSON 鋪進連線列，改為中文提示。
- 安裝包不再把使用者導向無關的 fork。
